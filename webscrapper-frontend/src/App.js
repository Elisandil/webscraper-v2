...
function App() {
  ...
  // Al montar, comprobamos sesión y resultados
  useEffect(() => {
    ...
    if (token) { 
      apiRequest('/results').then(({ ok, data }) => {
        if (ok) {
          ...
          // Si la petición falla, limpiamos la sesión
    ...
    // Un solo health check
    const iv = setInterval(() => apiRequest('/health'), 30000);
    return () => clearInterval(iv);
  }, []);

  const reloadResults = () =>
    apiRequest('/results').then(({ ok, data }) => ok && setResults(data.data || []));
 const handleLogout = () => {
   localStorage.removeItem('jwtToken');
   setUser(null);
   setResults([]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-100 overflow-x-hidden">
      ...
      <div className="container mx-auto px-6 py-8">
       ...
       <ResultsList
         ...
         onDelete={(id) => {
           setAlert({ type: 'success', message: 'Resultado eliminado con éxito' });
           reloadResults();
         }}
