...
export default function RegisterModal({ onAlert }) {
  ...
  // Abrimos modal cuando se hace click en #openRegisterBtn
  useEffect(() => {
  ...
  const submit = async (e) => {
    ...
    if (!username || !email || password.length < 6) {
      setError('Todos los campos son requeridos (mín. 6 caracteres en contraseña).');
      return;
    }
    ...
    if (ok) {
      onAlert({ type: 'success', message: 'Registro exitoso. Por favor inicia sesión.' });
      close();
    } else {
  ...
  if (!open) return null;
  return (
    <div id="registerModal" className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-gray-800 rounded-2xl shadow-2xl border border-white/10 w-full max-w-md p-6">
        <div className="flex justify-between mb-4">
          ...
          <button id="closeRegisterBtn" onClick={close}
        ...
        <form onSubmit={submit} id="registerForm" className="space-y-4">
          <div>
            <label htmlFor="regUsername" className="block mb-1 text-gray-300">Usuario</label>
          ...
          <div>
            <label htmlFor="regEmail" className="block mb-1 text-gray-300">Correo electrónico</label>
            <input id="regEmail" type="email" required
              value={email} onChange={e => setEmail(e.target.value)}
              className="w-full px-3 py-2 bg-gray-700 rounded focus:outline-none focus:ring" />
          </div>
          <div>
            <label htmlFor="regPassword" className="block mb-1 text-gray-300">Contraseña</label>
          ...
          {error && <div id="registerError" className="text-red-400 text-center">{error}</div>}
        </form>
      </div>
    </div>
  );
